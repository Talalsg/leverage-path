...
export default function Insights() {
  ...
  const handleCreate = async () => {
    ...
    const { error } = await supabase.from('insights').insert({ user_id: user.id, title: formData.title, content: formData.content || null });
    if (error) { toast({ title: 'Error', description: error.message, variant: 'destructive' }); }
    else { toast({ title: 'Insight added' }); setDialogOpen(false); setFormData({ title: '', content: '' }); fetchInsights(); }
  };

  const updateStatus = async (id: string, status: Status) => {
    const updates: any = { status };
    if (status === 'published') updates.publish_date = new Date().toISOString().split('T')[0];
        ...
        {insights.map(insight => (
          <Card key={insight.id} className="bg-card border-border/50">
            <CardContent className="p-5">
              ...
              <div className="mt-4">
                <Select value={insight.status} onValueChange={v => updateStatus(insight.id, v as Status)}>
